...
k=strmatch(cmd, c); % see if command matches anything in the structure - i.e., get the field name to work
if(isempty(k))
...
if(fmtc)
   ...
   if(rem(length(k), 2) == 0 && length(k) >= 2)
      ...
      if(~mflag) % SGL mode specified for this argument: just read the next number and move on
         ...
         v=str2num(arg); % convert the number
         try
            eval([pointer '.v = v;']); % store the new variable
         catch
