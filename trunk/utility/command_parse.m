...
if(strmatch(first, {'quit', 'exit'}, 'exact')) % block commands that might cause matlab to quit
    cmd = 'bye'; % replace with our own routine
    first = 'bye';
end;
lasterr = '';
c = strmatch(first, CMDS, 'exact'); % first search for those that are exact
%fprintf('first: <%s>,  c: %d', first, c);
%fprintf('\n');
if(isempty(c)) % no exact matches? then try for inexact matches
    c = strmatch(first, CMDS);
    if(isempty(c)) % not even inexact - then try internal commands
        try
