...
take_mode = 0;
if(nargin > 0)
    if(strmatch(flag, 'data'))
        da_mode = 1;
    end;
    if(strmatch(flag, 'take'))
        if(nargin == 2)
            take_mode = 1;
        else
            take_mode = 1;
...
switch(local_sf.update) % How do we prepare the stimulus?
    case 0 % compute the whole thing
        if(strcmp(local_sf.Method.v, 'noise')) %we do not store anything because the calculation is done on the fly
            ...
            [outdata, tbase, outrate, err] = eval([local_sf.Method.v '(local_sf)']); %#ok<NASGU> % generate the current stimulus with its method...
            if(err ~= 0)
            ...
            if(strmatch('vsco', fieldnames(outdata{1})))
    ...
    case 1 % it was already computed, so copy it to local areas for use. (do we really need to do this?)
        if(strcmp(local_sf.Method.v, 'noise')) %we do not store anything because the calculation is done on the fly
            ...
            for i = 1:length(local_sf.waveform)
                if(isempty(strmatch('v2', fieldnames(local_sf.waveform{i}))))
                    local_sf.waveform{i}.v2 = single(0 * local_sf.waveform{i}.v);
                end;
            end;
            if(isempty(strmatch('vsco', fieldnames(local_sf.waveform{1}))))
                local_sf.waveform{i}.vsco = single(local_sf.waveform{1}.v);
                local_sf.tbase{1}.vsco = local_sf.tbase{1}.v;
            end;
            if(isempty(strmatch('v2sco', fieldnames(local_sf.waveform{1}))))
                local_sf.waveform{1}.v2sco = single(local_sf.waveform{1}.v2);
            end;
        end;

    case 2 % in this case, we do the computation on the fly...
        [outdata, tbase, outrate, err] = eval([local_sf.Method.v '(local_sf, 1)']); %#ok<NASGU> % generate first stimulus of the group with its method...
        if(err ~= 0)
            return;
        end;
    ...
    otherwise
        for i = 1:length(local_sf.waveform)
            ...
            if(strmatch('v2', fieldnames(local_sf.waveform{i})))
        ...
        if(strmatch('vsco', fieldnames(local_sf.waveform{1})))
        ...
        if(strmatch('v2sco', fieldnames(local_sf.waveform{1})))
...
tmp_file = '';
if(~isempty(filename))
    [p, f] = fileparts(filename); %#ok<NASGU>
        ...
        for m = 1:m1
            for i = 1:nreps
                ...
                if(strcmpi(local_sf.Method.v, 'noise'))
                    ...
                    IN_ACQ = 0; %#ok<NASGU>
                    [outdata, tbase, outrate] = eval([local_sf.Method.v '(local_sf, m)']); %#ok<NASGU> % generate first stimulus of the group with its method...
                    IN_ACQ = ina;
                ...
                [t1, i1, gs1] = runNEURON(TEST_MODEL, duration, outrate, wave, ampmode);
                if(STOP_ACQ == 1)
                ...
                if(~isempty(t1))
                    gs2 = interp1(t1, gs1, time);
                    s1 = interp1(t1, i1, time);
                    u = find(isnan(s1));
                    s1(u) = 0; %#ok<FNDSB> % delete the nans - they are just zeros.
                    data = [gs2; s1]';
                else
...
swave = 0;
if(SCOPE_FLAG && ~isempty(strmatch('vsco', fieldnames(local_sf.waveform{1}))))
    swave = 1;
end;
    ...
    for m=1:m1 % for each variation of the waveform
        ...
        for i=1:nreps % for each rep of the waveform
            ...
            if(strcmp(local_sf.Method.v, 'noise')) % for waveforms computed on the fly (e.g., lots of individual ones)
                ...
                if(isempty(strmatch(fieldnames(outdata{1}), 'v2')))
                    outdata{1}.v2 = 0*outdata{1}.v;
                end;
                if(isempty(strmatch(fieldnames(outdata{1}), 'v2sco')))
